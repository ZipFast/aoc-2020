...
fn main() {
    if let Ok(lines) = read_lines("input") {
        ...
        for line in lines {
            ...
            let upper = &front[front.find("-").unwrap()+1..front.len()-2];
            println!("lower value is {}, upper value is {}", lower, upper);
            let (l, u) = (<usize as FromStr>::from_str(lower).unwrap(), <usize as FromStr>::from_str(upper).unwrap());
            let cs: Vec<_> = back.chars().collect();
            if (cs[l-1] == last_char && cs[u-1] != last_char) || (cs[l-1] != last_char && cs[u-1] == last_char) {
                success+=1;
            }
        }
        println!("the total success is {}", success);
